...
module.exports = class Cache {
  ...
  async refreshCache() {
    const { account, repository, pre, token, app_name } = this.config
    ...
    const release = data.find(item => {
      ...
      return !item.draft && isPre && item.name.includes(app_name +  ' v')
    })
    ...
    for (const asset of release.assets) {
      ...
      const signature = await getSignature(name, release.assets, headers)
